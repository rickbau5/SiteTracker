...
object Events {
  ...
  case class FindEntriesRequest(attributes: List[AnomalyDetail]) extends ServerRequest
  case class FindEntriesResponse(entries: Option[List[(SSystem, List[AnomalyEntry])]]) extends Response
  ...
  case class EditEntryRequest(entry: (SSystem, Identifier), attributes: List[AnomalyDetail]) extends ServerRequest with Mutator
  case class EditEntryResponse(updatedEntry: Option[AnomalyEntry]) extends Response
